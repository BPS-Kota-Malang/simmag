...
<section class="pt-3 pb-4" id="count-stats">
            ...
            <div class="card mb-4 p-3">
                <div class="card-body px-0 pt-0 pb-2">
                    <div class="table-responsive p-0">
                        ...
                        <a href="javascript:;" class="text-secondary font-weight-bold text-xs btn btn-primary mt-2" data-bs-toggle="modal" data-bs-target="#createDataUser">
                        ...
                        <table class="table table-hover table-bordered table-stripped align-items-center" id="users">
                            ...
                            <tr>
                                ...
                                <td>
                                    <a href="{{ route('user-management.reset-password', $data->id) }}" class="btn btn-info btn-xs" data-bs-toggle="modal" data-bs-target="#reset{{ $data->id }}">
                                        <i class="fas fa-key"></i>
                                    </a>

                                    <!-- <a href=# class="btn btn-info btn-xs" data-bs-toggle="modal"
                                    ...
                                    <a href="{{ route('user-management.destroy', $data->id) }}" onclick="notificationBeforeDelete(event, this)" class="btn btn-danger btn-xs">
                            ...
                            <!-- Modal Reset -->
                            <div class="modal fade" id="reset{{ $data->id }}" data-bs-backdrop="static" data-bs-keyboard="false" tabindex="-1" aria-labelledby="reset{{ $data->id }}" aria-hidden="true">
                                <div class="modal-dialog modal-dialog-centered">
                                    <div class="modal-content">
                                        <div class="modal-header">
                                            <h5 class="modal-title" id="reset{{ $data->id }}">Reset Password</h5>
                                            <button type="button" class="btn-close" data-bs-dismiss="modal" aria-label="Close"></button>
                                        </div>
                                        <form method="get" action="{{ route('user-management.reset-password', $data->id) }}">
                                            @csrf
                                            <div class="modal-body">
                                                Apakah Anda yakin ingin mereset password dari user tersebut?
                                            </div>
    ...
    <div class="modal fade" id="createDataUser" tabindex="-1" role="dialog" aria-labelledby="createDataUserLabel" aria-hidden="true">
        <div class="modal-dialog modal-lg" role="document">
            <div class="modal-content">
                <div class="modal-header">
                ...
                <form action="{{ route('user-management.store') }}" method="POST">
                    ...
                    <div class="modal-body">
                        <div class="form-group">
                            <label for="name">Nama</label>
                            <input type="text" class="form-control @error('name') is-invalid @enderror" id="name" placeholder="Nama" name="name" value="{{ old('name') }}" required>
                            @error('name')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
                        ...
                        <div class="form-group">
                            <label for="email">E-mail</label>
                            <input type="email" class="form-control @error('email') is-invalid @enderror" id="email" placeholder="Masukkan Email" name="email" value="{{ old('email') }}" required>
                            @error('email')
                            <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </div>
